...
-- F1.1 NAO DEVE: saldo insuficiente falha e mantem o saldo
...
SELECT s.name, b.competencia, b.minutos_consumidos FROM studio_billing b JOIN studios s ON s.id = b.id_studio ORDER BY s.id;
...
SELECT tabela, operacao, usuario FROM audit_log WHERE tabela = 'profiles' ORDER BY id DESC LIMIT 1;

-- F2.4 DEVE: insert sai com nome padronizado
INSERT INTO customers (full_name, email, password_hash, cpf, state_uf, date_of_birth) VALUES ('  ana beatriz  ', 'ANA@EXAMPLE.COM', 'hashsenha000', '222.333.444-55', 'SP', '1998-04-01');
...
SELECT total_acessos FROM reproduction_summary WHERE id = 1;
CALL registrar_reproducao(2, 3, '10.1.1.1', 2, @rid2);
SELECT total_acessos FROM reproduction_summary WHERE id = 1;

-- F2.6 DEVE: completar ate 5 perfis funciona
INSERT INTO profiles (display_name, id_customer) VALUES ('Extra 1', 1), ('Extra 2', 1), ('Extra 3', 1);
-- F2.6 NAO DEVE: o 6o perfil falha
INSERT INTO profiles (display_name, id_customer) VALUES ('Extra 6', 1);
-- F2.6 DEVE: outro assinante ainda pode criar
INSERT INTO profiles (display_name, id_customer) VALUES ('Extra Pedro', 3);
