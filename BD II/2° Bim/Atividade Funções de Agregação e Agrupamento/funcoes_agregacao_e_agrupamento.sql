...
-- 1
SELECT
SUM(ip.quantidade * prod.valor_unitario) AS faturamento_total
...
SELECT
prod.unidade,
AVG(prod.valor_unitario) AS valor_medio
FROM produtos prod WHERE prod.unidade = 'L';

-- 3
SELECT
c.uf AS estado,
COUNT(c.uf) AS quantidade_estado
FROM clientes c
GROUP BY c.uf
