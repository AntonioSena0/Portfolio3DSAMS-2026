<x-app-layout>
    <section class="bg-[#F5F5F5] py-14">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <h1 class="mt-2 text-4xl font-bold text-[#313131]">Editar aluno</h1>
        </div>
    </section>

    <section class="py-14">
        <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
            <form action="{{ route('admin.students.update', $user) }}" method="POST" class="bg-white p-8 shadow-sm">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="course_id" id="course_id" required
                            ...
                            @foreach ($courses as $course)
                                <option value="{{ $course->id }}" @selected(old('course_id', $user->course_id) == $course->id)>{{ $course->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="grade" id="grade" required
                            ...
                            <option value="1" @selected(old('grade', $user->grade) == '1')>1º Ano</option>
                            <option value="2" @selected(old('grade', $user->grade) == '2')>2º Ano</option>
                            <option value="3" @selected(old('grade', $user->grade) == '3')>3º Ano</option>
                        </select>
                        @error('grade') <p class="mt-1 text-sm text-[#B20000]">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="password" class="block text-sm font-bold text-[#313131]">Nova senha <span class="font-normal text-[#5A5A5A]">(deixe em branco para manter)</span></label>
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-sm font-bold text-[#313131]">Confirmar nova senha</label>
